...
const productAssignmentSchema = z.object({
    ...
    franchiseId: z.preprocess(
        ...
        z.string().regex(/^\d+$/, "Franchise ID must be a number").optional()
    ),
    ...
    .refine(data => {
        if (data.customerType === 'FRANCHISE') {
            return data.merchantIds?.length > 0 && !!data.franchiseId
        }

        if (data.customerType === 'MERCHANT') {
            return data.merchantIds?.length > 0 && !data.franchiseId
        }
...
const ProductAssignmentFormModal = ({ onCancel, onSubmit, initialData = null, isEdit = false }) => {
    ...
    useEffect(() => {
        if (isEdit && initialData && !dataInitialized) {
            reset({
                ...
                merchantIds: initialData.merchantIds?.map(String) || [],
    ...
    // Fetch merchants under selected franchise
    useEffect(() => {
        if (
            watchedFields[0] === 'FRANCHISE' &&
            watchedFields[1]   // franchiseId
        ) {
            api.get(`/merchants/franchise/${watchedFields[1]}`)
    ...
    useEffect(() => {
        if (watchedFields[0] === 'MERCHANT') {
            setValue('franchiseId', undefined)
        }
    }, [watchedFields[0]])

    // Fetch products when merchant is selected (FRANCHISE or MERCHANT flow)
    useEffect(() => {
    ...
    useEffect(() => {
        const fetchPricingSchemes = async () => {
            const productId = watchedFields[3]   // productId index
            ...
            try {
                ...
                const apiCustomerType =
                    customerType === 'FRANCHISE' ? 'franchise' : 'direct_merchant'
    ...
    // Clear merchant & below when franchise changes
    useEffect(() => {
        if (!dataInitialized || isEdit) return
            ...
            const payload = data.merchantIds.map((merchantId) => ({
                ...
                franchiseId:
                    data.customerType === 'FRANCHISE'
                        ...
                        : null,
                merchantId: Number(merchantId),   // SINGLE merchant
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-gray-50 p-4 rounded-lg">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                {/* MERCHANT DROPDOWN */}
                                {(
                                    watchedFields[0] === 'MERCHANT' ||
                                    (watchedFields[0] === 'FRANCHISE' && watchedFields[1])
                                ) && (
                                        <div>
